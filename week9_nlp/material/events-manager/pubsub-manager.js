...
class PubSubManager {
    ...
    publish(topic, message){
        this.log('Publishing message ' +  this.msgToStr(message) + ' for topic ' + topic);
        for (let index in this.subscribers) {
            if (this.subscribers[index].topic == topic) {
                this.log('Message for topic ' + topic + ' received by subscriber ' + this.subscribers[index].id)
    ...
    subscribe(topic, callback){
        ...
        this.log('Created new subscriber for topic ' + topic + ' with id ' + this.lastId);
        return this.lastId;
    }
    ...
    unsubscribe(subscriberId) {
        for (let index in this.subscribers){
            if (this.subscribers[index].id == subscriberId) {
                this.log('Unsubscribing subscriber with id ' + subscriberId + ' for topic ' + this.subscribers[index].topic);
